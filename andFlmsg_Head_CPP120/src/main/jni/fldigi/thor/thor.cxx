...
// n == 1, 2, 3, 4 ... THORMAXFFTS
    if (n == 0)
        ...
        f = THORFIRSTIF - THORBASEFREQ - bandwidth * 0.5 +
            (samplerate / symlen) * ((double) n / paths);
...
void thor::softdecodesymbol() {
    ...
    c = (int) floor(fdiff + .5);
    {
        if (c < -16 || 0 == c || 1 == c || c > 17)
            outofrange = true; // Out of the range of the function thor::sendsymbol()
        if (progdefaults.THOR_PREAMBLE) {
            if (preambledetect(c)) {
                softflushrx(); // Flush the soft rx pipeline with punctures (interleaver & viterbi decoder)
                lastmag = 0;
...
//Android no squelch yet	if (progStatus.sqlonoff && metric < progStatus.sldrSquelchValue) nowmag = 0;
    ...
    lastsymbols[3] = (lastc & 1) == 1 ? one : zero;
    lastc /= 2;
    lastsymbols[2] = (lastc & 1) == 1 ? one : zero;
    lastc /= 2;
    lastsymbols[1] = (lastc & 1) == 1 ? one : zero;
    lastc /= 2;
    lastsymbols[0] = (lastc & 1) == 1 ? one : zero;
    lastc /= 2;
...
//Android old complex class		  	x = abs(pipe[pipeptr].vector[i]);
    ...
    } while (nextCWI[symbol] && soft_symbol_trycount <
                                SoftBailout); // Run while the detected symbol has been identified as CWI (alt: bailout after 6 trys)
